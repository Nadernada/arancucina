...
export const OurValuesBlock: React.FC<OurValuesBlockType> = ({ content }) => {
  return (
    <div className="container flex flex-row justify-between px-16 items-center">
      {content?.map((item, i) => {
        return (
          <div key={i} className="flex flex-row justify-start items-center gap-12 w-[30%]">
            ...
            <Media resource={item.image} imgClassName="w-24 h-24 aspect-square max-w-none" />
          </div>
        )
      })}
    </div>
